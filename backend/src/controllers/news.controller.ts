...
export class NewsController {
  ...
  static async toggleBookmark(
    ...
    try {
      ...
      const result = await NewsService.toggleBookmark(articleId, userId);
  ...
  static async getArticleSummary(
    ...
    try {
      ...
      const summary = await NewsService.getArticleSummary(articleId);
  ...
  static async getCoverageComparison(
    req: AuthRequest,
    res: Response,
    next: NextFunction,
  ): Promise<void> {
    try {
      ...
      const comparison =
        await NewsService.getEventGroupComparison(eventGroupId);
